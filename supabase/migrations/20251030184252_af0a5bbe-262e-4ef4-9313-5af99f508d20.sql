...
CREATE POLICY "Users can view all seeker profiles"
...
CREATE POLICY "Users can view all company profiles"
...
CREATE POLICY "Companies can view CVs of applicants"
  ON cvs FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM applications a
      JOIN jobs j ON a.job_id = j.id
      WHERE a.cv_id = cvs.id AND j.company_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view own applications"
  ...
  USING (auth.uid() = seeker_id OR auth.uid() IN (SELECT company_id FROM jobs WHERE id = job_id));
...
CREATE POLICY "Users can view own CVs in storage"
